...
;;
;; test/clj/monads/test/core_test.clj @ Tue Jul 23 19:45:42 PDT 2013
;;
...
(deftest test-do (it "" (is (= [19 {:val 19}] (clojure.core/deref ((monadic/do parse-m [_ (m/set-val :val 19)] 19) {}))))) (it "" (let [tinc (fn* [p1__1297#] (vector (inc p1__1297#)))] (is (= [[1 2 3] [3 4 5]] (monadic/do list [a (range 5) :when (odd? a) x (tinc a) y (tinc x)] [a x y]))))))
(deftest test-hash-set-writer (it "" (let [test-m (m/writer-t hash-set []) writer-m (m/writer []) write-msg (fn [msg] (writer-transformer. hash-set (hash-set ((m/writer [msg]) nil)) writer-m)) listen-msgs (fn [mv] (writer-transformer. hash-set (->> (clojure.core/deref mv) (map (fn* [p1__1298#] (m/listen p1__1298#))) set) writer-m)) censor-msgs (fn [f mv] (writer-transformer. hash-set (->> (clojure.core/deref mv) (map (fn* [p1__1299#] (m/censor f p1__1299#))) set) writer-m))] (is (= [nil [:msg1]] (clojure.core/deref (first (clojure.core/deref (write-msg :msg1)))))) (is (= [[nil [:msg3]] [:msg3]] (clojure.core/deref (first (clojure.core/deref (listen-msgs (write-msg :msg3))))))) (is (= [[nil [nil [:msg3]] nil] [:msg1 :msg3 :msg2 :msg4]] (->> (m/seq [(write-msg :msg1) (listen-msgs (write-msg :msg3)) (write-msg :msg2)]) (censor-msgs (fn* [p1__1300#] (conj p1__1300# :msg4))) deref first deref))) (is (= #{[nil [:msg1 :msg3]] [5 [:msg3]]} (->> (m/plus [(write-msg :msg1) (m/zero (test-m nil)) (m/zero (write-msg :msg2)) (test-m 5)]) (censor-msgs (fn* [p1__1301#] (conj p1__1301# :msg3))) deref (map deref) set))))))
(deftest test-state-writer-maybe (it "" (let [test-m (m/state-t (m/writer-t m/maybe [])) writer-m (m/writer-t m/maybe []) write-msg (fn [msg] (state-transformer. writer-m nil ((m/state-t (m/writer-t m/maybe [msg])) nil) (constantly (test-m nil)) nil)) listen-msgs (fn [mv] (state-transformer. writer-m nil (fn [s] (let [[[_ s] msgs] (clojure.core/deref (clojure.core/deref (clojure.core/deref (mv s))))] ((m/writer-t m/maybe msgs) [msgs s]))) (fn [v] (test-m v)) nil)) censor-msgs (fn [f mv] (state-transformer. writer-m nil (fn [s] (let [[[v s] msgs] (clojure.core/deref (clojure.core/deref (clojure.core/deref (mv s))))] ((m/writer-t m/maybe (f msgs)) [[v msgs] s]))) (fn [v] (test-m v)) nil))] (is (= [[nil :state] [:msg]] (clojure.core/deref (clojure.core/deref (clojure.core/deref ((write-msg :msg) :state)))))) (is (= [[:result :state] [:msg]] (clojure.core/deref (clojure.core/deref (clojure.core/deref ((m/bind (write-msg :msg) (fn [x] (is (nil? x)) (test-m :result))) :state)))))) (is (= [[[nil nil] :state] [:msg1 :msg2]] (clojure.core/deref (clojure.core/deref (clojure.core/deref ((m/seq [(write-msg :msg1) (write-msg :msg2)]) :state)))))) (is (= [[nil :state] [:msg1]] (clojure.core/deref (clojure.core/deref (clojure.core/deref ((m/plus [(write-msg :msg1) (write-msg :msg2)]) :state)))))) (is (= [[nil :state] [:msg2]] (clojure.core/deref (clojure.core/deref (clojure.core/deref ((m/plus [(m/zero (test-m nil)) (write-msg :msg2)]) :state)))))) (is (= [[[:msg3] :state] [:msg3]] (clojure.core/deref (clojure.core/deref (clojure.core/deref ((listen-msgs (write-msg :msg3)) :state)))))) (is (= [[[[nil [:msg3] nil] [:msg1 :msg3 :msg2]] :state] [:msg1 :msg3 :msg2 :msg4]] (clojure.core/deref (clojure.core/deref (clojure.core/deref ((->> (m/seq [(write-msg :msg1) (listen-msgs (write-msg :msg3)) (write-msg :msg2)]) (censor-msgs (fn* [p1__1302#] (conj p1__1302# :msg4)))) :state)))))) (is (= [[[nil [:msg1]] :state] [:msg1 :msg3]] (clojure.core/deref (clojure.core/deref (clojure.core/deref ((->> (m/plus [(m/zero (test-m nil)) (m/zero (write-msg :msg2)) (write-msg :msg1)]) (censor-msgs (fn* [p1__1303#] (conj p1__1303# :msg3)))) :state)))))))))
(deftest domonad-if-then (it "" (let [monad-value (monadic/do m/maybe [a (m/maybe 5) :let [c 7] :if (and (= a 5) (= c 7)) :then [b (m/maybe 6)] :else [b m/maybe-zero-val]] [a b])] (is (= (clojure.core/deref monad-value) [5 6])))))
(deftest domonad-nested-if-then (it "" (let [monad-value (monadic/do m/maybe [a (m/maybe 5) :if (= a 5) :then [b (m/maybe 6) :if (= b 6) :then [c (m/maybe 7)] :else [c m/maybe-zero-val]] :else [b m/maybe-zero-val c m/maybe-zero-val]] [a b c])] (is (= (clojure.core/deref monad-value) [5 6 7])))))
(deftest domonad-if-then-with-when (it "" (let [monad-value (monadic/do m/maybe [a (m/maybe 5) :when (= a 5) :if (= a 1) :then [b (m/maybe 6)] :else [b m/maybe-zero-val]] [a b])] (is (= monad-value m/maybe-zero-val)))))
(deftest domonad-cond (it "" (let [monad-value (monadic/do m/maybe [a (m/maybe 5) :when (= a 5) :cond [(< a 1) [result (m/maybe "less than one")] (< a 3) [result (m/maybe "less than three")] (< a 6) [result (m/maybe "less than six")] :else [result (m/maybe "arbitrary number")]] b (m/maybe 7) :let [some-val 12345]] [result b some-val])] (is (= (clojure.core/deref monad-value) ["less than six" 7 12345])))))